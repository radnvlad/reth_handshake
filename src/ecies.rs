...
impl Ecies {
    ...
    pub fn encrypt<'a>(&self, data_to_encrypt: BytesMut) -> Result<BytesMut, &'static str>  
    {
        ...
        let total_size = u16::try_from(PUBLIC_KEY_SIZE + IV_SIZE + data_to_encrypt.len() + TAG_SIZE).unwrap();
        ...
        //===self.encrypt(auth_body, &mut buf)rem;
    }
    ...
    pub fn get_secrets(&self ) {
        ...
        let ephemeral_key = Self::agree(self.ephemeral_remote_pub_key.unwrap(),self.ephemeral_priv_key);
